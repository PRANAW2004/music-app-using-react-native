...
export default function Folk({navigation}){
    ...
    async function otherlikedvalueselect(){
    ...
    const currentPlaying = useCallback(async () => {
        ...
        for(var i=0;i<otherdata.length;i++){
            if(JSON.parse(currentplaying) === otherdata[i]['title']){
                setcurrentPlaying(otherdata[i]['id']);
                AsyncStorage.setItem("current-playing-num",JSON.stringify(otherdata[i]['id']));
            }
        }
    }
    });
        ...
        if(id >= otherdata.length){
            ...
            for(var i=0;i<otherdata.length;i++){
                if(otherdata[i]['id'] === id){
                    await AsyncStorage.setItem('current-playing-song',JSON.stringify(otherdata[i]['title']));
                    AsyncStorage.setItem("current-playing",JSON.stringify(otherdata[i]['title']));
                    //AsyncStorage.setItem("current-genre",JSON.stringify('folk'));

                    for(var o=0;o<alldata.length;o++){
                        if(otherdata[i]['title'] === alldata[o]['title']){
                            AsyncStorage.setItem("alldata-playing-num",JSON.stringify(alldata[o]['id']));
                        }
                    }

                    if(history.length > 50){
                        sethistory((otherdata) => otherdata.filter((_,index) => index !== 0));
                    }else{
                        let date = new Date().toLocaleDateString();
                        let date1 = date;
                        // console.log(date1);
                        let data11 = date1 + ":"+otherdata[i]['title']
                        sethistory((current) => [...current,data11]);
                    }
                    // await AsyncStorage.setItem("history",JSON.stringify(history));
                    ...
                    try{
                        if(i === 0){
                            for(j=i+1;j<otherdata.length;j++){
                                arr.push(otherdata[i+j]);
                            }
                        }
                        else{
                        for(j=i;j<otherdata.length-1;j++){
                            arr.push(otherdata[j+1]);
                        }
                        }
    ...
    async function liked(genre,title){
        if(genre === "other"){
        for(var i=0;i<otherdata.length;i++){
            if(otherdata[i]['title'] === title){

                otherdata[i]['liked'] = otherdata[i]['liked'] === 'cards-heart'?'cards-heart-outline':'cards-heart';
                otherdata[i]['color'] = otherdata[i]['color'] === 'red'?'white':'red';
        ...
        <View style={styles.folkview}>
            ...
            {otherdata.map((e)=>{
                ...
                <View style={{flex:1,width:'100%',display:"flex",justifyContent:"center"}}>
                    <Pressable style={{width:'100%',display:"flex",alignItems:"center"}} onPress={async ()=>{await AsyncStorage.setItem("genre",JSON.stringify("other"));play1(e['id']);setcurrentPlaying(e['id']);}}>
                    {/* <Pressable> */}
