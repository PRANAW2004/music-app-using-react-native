...
export default function Folk({navigation}){
    ...
    async function hindilikedvalueselect(){
    ...
    const currentPlaying = useCallback(async () => {
        ...
        for(var i=0;i<hindidata.length;i++){
            if(JSON.parse(currentplaying) === hindidata[i]['title']){
                setcurrentPlaying(hindidata[i]['id']);
                AsyncStorage.setItem("current-playing-num",JSON.stringify(hindidata[i]['id']));
            }
        }
    }
    });
    ...
    async function play1(id){
        ...
        if(id >= hindidata.length){
            ...
            for(var i=0;i<hindidata.length;i++){
                if(hindidata[i]['id'] === id){
                    await AsyncStorage.setItem('current-playing-song',JSON.stringify(hindidata[i]['title']));
                    AsyncStorage.setItem("current-playing",JSON.stringify(hindidata[i]['title']));
                    //AsyncStorage.setItem("current-genre",JSON.stringify('folk'));

                    for(var o=0;o<alldata.length;o++){
                        if(hindidata[i]['title'] === alldata[o]['title']){
                            AsyncStorage.setItem("alldata-playing-num",JSON.stringify(alldata[o]['id']));
                        }
                    }
                    ...
                    if(history.length > 50){
                        sethistory((hindidata) => hindidata.filter((_,index) => index !== 0));
                    }else{
                        let date = new Date().toLocaleDateString();
                        let date1 = date;
                        // console.log(date1);
                        let data11 = date1 + ":"+hindidata[i]['title']
                        sethistory((current) => [...current,data11]);
                    }
                    // await AsyncStorage.setItem("history",JSON.stringify(history));
                    ...
                    try{
                        if(i === 0){
                            for(j=i+1;j<hindidata.length;j++){
                                arr.push(hindidata[i+j]);
                            }
                        }
                        else{
                        for(j=i;j<hindidata.length-1;j++){
                            arr.push(hindidata[j+1]);
                        }
                        }
    ...
    async function liked(genre,title){
        if(genre === "hindi"){
        for(var i=0;i<hindidata.length;i++){
            if(hindidata[i]['title'] === title){

                hindidata[i]['liked'] = hindidata[i]['liked'] === 'cards-heart'?'cards-heart-outline':'cards-heart';
                hindidata[i]['color'] = hindidata[i]['color'] === 'red'?'white':'red';
                // console.log(data[i]['liked'])
                if(hindidata[i]['liked'] === 'cards-heart'){
                    setlikedsong(current => [...current,hindidata[i]['title']]);
                }
                else{
        ...
        <View style={styles.folkview}>
            ...
            {hindidata.map((e)=>{
                ...
                <View style={{flex:1,width:'100%',display:"flex",justifyContent:"center"}}>
                    <Pressable style={{width:'100%',display:"flex",alignItems:"center"}} onPress={async ()=>{await AsyncStorage.setItem("genre",JSON.stringify("hindi"));play1(e['id']);setcurrentPlaying(e['id']);}}>
                    {/* <Pressable> */}
                    ...
                    <Pressable onPress={() => {liked("hindi",e['title']),setsonglikedbool(true)}}>
