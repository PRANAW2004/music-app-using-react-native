import { View, Text,StyleSheet,BackHandler,ScrollView,Image, Pressable,Modal,ActivityIndicator,Dimensions } from 'react-native';
...
export default function Folk({navigation}){
  ...
  let width = Dimensions.get('window').width;
    ...
    async function englishlikedvalueselect(){
    ...
    async function play1(id){
        ...
        if(id >= englishdata.length){
            ...
            for(var i=0;i<englishdata.length;i++){
                if(englishdata[i]['id'] === id){
                    await AsyncStorage.setItem('current-playing-song',JSON.stringify(englishdata[i]['title']));
                    AsyncStorage.setItem("current-playing",JSON.stringify(englishdata[i]['title']));
                    //AsyncStorage.setItem("current-genre",JSON.stringify('folk'));

                    for(var o=0;o<alldata.length;o++){
                        if(englishdata[i]['title'] === alldata[o]['title']){
                            AsyncStorage.setItem("alldata-playing-num",JSON.stringify(alldata[o]['id']));
                        }
                    }
                    ...
                    if(history.length > 50){
                        sethistory((englishdata) => englishdata.filter((_,index) => index !== 0));
                    }else{
                        let date = new Date().toLocaleDateString();
                        let date1 = date;
                        // console.log(date1);
                        let data11 = date1 + ":"+englishdata[i]['title']
                        sethistory((current) => [...current,data11]);
                    }
                    // await AsyncStorage.setItem("history",JSON.stringify(history));

                    let arr = [englishdata[i]];
                    try{
                        if(i === 0){
                            for(j=i+1;j<englishdata.length;j++){
                                arr.push(englishdata[i+j]);
                            }
                        }
                        else{
                        for(j=i;j<englishdata.length-1;j++){
                            arr.push(englishdata[j+1]);
                        }
                        }
        ...
        <View style={styles.folkview}>
            ...
            {englishdata.map((e)=>{
                ...
                <View style={{flex:1,width:'100%',display:"flex",justifyContent:"center"}}>
                    <Pressable style={{width:'100%',display:"flex",alignItems:"center"}} onPress={async ()=>{await AsyncStorage.setItem("genre",JSON.stringify("english"));play1(e['id']);setcurrentPlaying(e['id']);}}>
                    ...
                    <Pressable onPress={() => {liked("english",e['title']),setsonglikedbool(true)}}>
