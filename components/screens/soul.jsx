...
export default function Folk({navigation}){
  ...
  let width = Dimensions.get('window').width;
    ...
    async function soullikedvalueselect(){
    ...
    const likeddata = useCallback(async() => {
        ...
        // for(var i=0;i<souldata.length;i++){
        //     souldata[i]['liked'] = 'cards-heart-outline';
        //     souldata[i]['color'] = 'white';
        // }
        ...
        if(arr.length === 0){
            for(var i=0;i<souldata.length;i++){
                if(souldata[i]['liked'] === 'cards-heart'){
                    souldata[i]['liked'] = 'cards-heart-outline';
                    souldata[i]['color'] = 'white';
                }
            }
        ...
        for(var i=0;i<souldata.length;i++){
            for(var j=0;j<arr.length;j++){
                if(souldata[i]['title'] === arr[j]){
                    souldata[i]['liked'] = 'cards-heart';
                    souldata[i]['color'] = 'red';
                    break;
                }else{
                    souldata[i]['liked'] = 'cards-heart-outline';
                    souldata[i]['color'] = 'white';
    ...
    const currentPlaying = useCallback(async () => {
        ...
        for(var i=0;i<souldata.length;i++){
            if(JSON.parse(currentplaying) === souldata[i]['title']){
                setcurrentPlaying(souldata[i]['id']);
                AsyncStorage.setItem("current-playing-num",JSON.stringify(souldata[i]['id']));
            }
        }
    }
    });
    ...
    async function play1(id){
        ...
        if(id >= souldata.length){
            ...
            for(var i=0;i<souldata.length;i++){
                if(souldata[i]['id'] === id){
                    await AsyncStorage.setItem('current-playing-song',JSON.stringify(souldata[i]['title']));
                    AsyncStorage.setItem("current-playing",JSON.stringify(souldata[i]['title']));
                    //AsyncStorage.setItem("current-genre",JSON.stringify('folk'));

                    for(var o=0;o<alldata.length;o++){
                        if(souldata[i]['title'] === alldata[o]['title']){
                            AsyncStorage.setItem("alldata-playing-num",JSON.stringify(alldata[o]['id']));
                        }
                    }

                    if(history.length > 50){
                        sethistory((souldata) => souldata.filter((_,index) => index !== 0));
                    }else{
                        let date = new Date().toLocaleDateString();
                        let date1 = date;
                        // console.log(date1);
                        let data11 = date1 + ":"+souldata[i]['title']
                        sethistory((current) => [...current,data11]);
                    }
                    // await AsyncStorage.setItem("history",JSON.stringify(history));
                    ...
                    try{
                        if(i === 0){
                            for(j=i+1;j<souldata.length;j++){
                                arr.push(souldata[i+j]);
                            }
                        }
                        else{
                        for(j=i;j<souldata.length-1;j++){
                            arr.push(souldata[j+1]);
                        }
                        }
    ...
    async function liked(genre,title){
        if(genre === "soul"){
        for(var i=0;i<souldata.length;i++){
            if(souldata[i]['title'] === title){

                souldata[i]['liked'] = souldata[i]['liked'] === 'cards-heart'?'cards-heart-outline':'cards-heart';
                souldata[i]['color'] = souldata[i]['color'] === 'red'?'white':'red';
                // console.log(souldata[i]['liked'])
                if(souldata[i]['liked'] === 'cards-heart'){
                    setlikedsong(current => [...current,souldata[i]['title']]);
                }
                else{
        ...
        <View style={styles.folkview}>
            ...
            {souldata.map((e)=>{
                ...
                <View style={{flex:1,width:'100%',display:"flex",justifyContent:"center"}}>
                    <Pressable style={{width:'100%',display:"flex",alignItems:"center"}} onPress={async ()=>{await AsyncStorage.setItem("genre",JSON.stringify("soul"));play1(e['id']);setcurrentPlaying(e['id']);}}>
                    ...
                    <Pressable onPress={() => {liked("soul",e['title']),setsonglikedbool(true)}}>
