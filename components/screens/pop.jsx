...
export default function Folk({navigation}){
  ...
  let width = Dimensions.get('window').width;
    ...
    async function poplikedvalueselect(){
    ...
    const likeddata = useCallback(async() => {
        ...
        if(arr.length === 0){
            for(var i=0;i<popdata.length;i++){
                if(popdata[i]['liked'] === 'cards-heart'){
                    popdata[i]['liked'] = 'cards-heart-outline';
                    popdata[i]['color'] = 'white';
                }
            }
    ...
    const currentPlaying = useCallback(async () => {
        ...
        for(var i=0;i<popdata.length;i++){
            if(JSON.parse(currentplaying) === popdata[i]['title']){
                setcurrentPlaying(popdata[i]['id']);
                AsyncStorage.setItem("current-playing-num",JSON.stringify(popdata[i]['id']));
            }
        }
    }
    });
        ...
        if(id >= popdata.length){
            ...
            for(var i=0;i<popdata.length;i++){
                if(popdata[i]['id'] === id){
                    ...
                    for(var o=0;o<alldata.length;o++){
                        if(popdata[i]['title'] === alldata[o]['title']){
                            AsyncStorage.setItem("alldata-playing-num",JSON.stringify(alldata[o]['id']));
                        }
                    }
    ...
    async function liked(genre,title){
        if(genre === "pop"){
        for(var i=0;i<popdata.length;i++){
            if(popdata[i]['title'] === title){

                popdata[i]['liked'] = popdata[i]['liked'] === 'cards-heart'?'cards-heart-outline':'cards-heart';
                popdata[i]['color'] = popdata[i]['color'] === 'red'?'white':'red';
                // console.log(popdata[i]['liked'])
                if(popdata[i]['liked'] === 'cards-heart'){
                    setlikedsong(current => [...current,popdata[i]['title']]);
                }
                else{
        ...
        <View style={styles.folkview}>
            ...
            {popdata.map((e)=>{
                ...
                <View style={{flex:1,width:'100%',display:"flex",justifyContent:"center"}}>
                    ...
                    <Pressable onPress={() => {liked("pop",e['title']),setsonglikedbool(true)}}>
